...
/* Ownership only, never content or hashes: these files existed before the installer touched them. */
function validatePreservedTargets(cfg) {
  if (!('preservedTargets' in cfg)) return;
  if (!Array.isArray(cfg.preservedTargets) || cfg.preservedTargets.some(t => !KNOWN.includes(t) || !Array.isArray(cfg.targets) || !cfg.targets.includes(t))
      || new Set(cfg.preservedTargets).size !== cfg.preservedTargets.length)
    die('config: "preservedTargets" must be a unique list of installed target names');
}
...
/* Do the notes pointers in a rendered file name something that is there? The README is the file the
   AI is told to read before it writes, and the level-2 scaffold creates it with the folder, so its
   presence is the sharpest one-stat answer. The LEVEL is not: a hand-written config carries none, a
   level-1 install can land beside an existing notes folder, and a re-run at a lower level does not
   delete what a higher one made (round 1, findings 3 to 5). Cloud answers have no local folder at
   all, and notesPending() short-circuits on that before this is consulted. */
const notesScaffolded = (root, answers) => {
  if (!answers) return true;
  try { return fs.statSync(path.join(root, onboarding.baseFor(answers), 'README.md')).isFile(); }
  catch (_) { return false; }
};

function renderTarget(key, target, profile, rules, cfg, root) {
  if (target.source === 'onboarding') {
    ...
    return onboarding.renderOnboarding(cfg.onboarding, { notesScaffolded: notesScaffolded(root, cfg.onboarding) });
  }
  ...
  if (target.boxes) {
    ...
    const box2 = answers ? [onboarding.contractBlock(answers, { files: false, notesScaffolded: notesScaffolded(root, answers) }).split('\n').slice(2).join('\n'), ruleText].filter(Boolean).join('\n\n') : ruleText;
    const over = [];
...
function main() {
  ...
  if (contract) {
    ...
    if (cfgC && cfgC.onboarding && withPersonal) out.push(onboarding.contractBlock(cfgC.onboarding, { notesScaffolded: notesScaffolded(root, cfgC.onboarding) }), '');
  ...
  const plan = targets.map(key => {
    ...
    const block = renderTarget(key, target, profile, rules, cfg, root);
    let existing = null;
  ...
  recordInstalled(root, plan);
}

/* After a successful commit, record the exact bytes just written for each rendered target, so
   --uninstall can recognize them later even after this package has moved on to a newer release.
   For a target this run OWNS outright (not in the stored preservedTargets, i.e. not a file that
   already existed before its target was first installed), the whole file is generated by this
   tool, so the whole file's bytes are recorded. For a pre-existing home file, only the marker
   block belongs to this tool, so only the block is hashed; the surrounding bytes are the user's
   and are never fingerprinted. Read only; never runs during --check (nothing was written), and
   a config predating this field, or none at all, is left exactly as it is. */
function recordInstalled(root, plan) {
  const cfgPath = path.join(root, '.agent-personalizer.json');
  let raw;
  try { raw = JSON.parse(fs.readFileSync(cfgPath, 'utf8')); } catch (_) { return; }
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return;
  const installed = (raw.installed && typeof raw.installed === 'object' && !Array.isArray(raw.installed)) ? { ...raw.installed } : {};
  for (const p of plan) {
    const preserved = !p.raw && Array.isArray(raw.preservedTargets) && raw.preservedTargets.includes(p.key);
    installed[p.target.file] = sha256(Buffer.from(preserved ? p.block : p.output));
  }
...
/* Pure output planning for uninstall. Uses this package's renderer, never executable code from
   the installed folder. The caller preflights all paths before asking for these bytes. */
function renderedContents(root, cfg) {
  const rules = loadRules(root, null);
  const profile = loadProfile(root);
  renderTarget.boxes = null;
  renderTarget.overBudget = [];
  return cfg.targets.flatMap(key => {
    const target = TARGETS[key];
    if (!target) die(`unknown target "${key}"`);
    safeTargetPath(root, target.file, key);
    const block = renderTarget(key, target, profile, rules, cfg, root);
    const out = [{ key, rel: target.file, block }];
    if (target.boxes) for (const [which, rel] of Object.entries(target.boxFiles || {})) {
      safeTargetPath(root, rel, `${key}:${which}`);
      out.push({ key: `${key}:${which}`, rel, block: renderTarget.boxes[which], raw: true });
    }
    return out;
  });
}
...
module.exports = { main, fenceMap, markerState, splice, between, preflightSources, parseFrontmatter, parseSections, readConfig, validatePreservedTargets, renderedContents, Refusal, DIE_THROWS: false };
