...
function validate(input) {
  ...
  for (const q of QUESTIONS) {
    ...
    const v = input[q.id];
    if (q.type === 'text') {
      if (typeof v !== 'string' || v.length > 400 || /[\r\n]/.test(v)) throw new Error(`"${q.id}" must be one line of text (up to 400 characters)`);
      out[q.id] = v.trim();
      ...
      if (!Array.isArray(v) || v.some(s => typeof s !== 'string' || /[\r\n]/.test(s) || s.length > 200) || v.length > 50) throw new Error(`"${q.id}" must be a list of up to 50 short strings`);
...
- **Name and pronouns:** ${esc(a.name)}${a.pronouns ? `, ${esc(a.pronouns)}` : ''}
- **What I do:** ${a.work ? esc(a.work) : '(fill in)'}
- **Current focus:** ${a.focus ? esc(a.focus) : '(fill in)'}
- **Off limits:** ${a.off_limits.length ? a.off_limits.map(esc).join(', ') : 'nothing declared yet'}
...
- **Never:** ${a.never.length ? a.never.map(esc).join('; ') : 'nothing declared yet'}
...
_Generated by agent-personalizer from ${esc(a.name)}'s own answers (stored in \`.agent-personalizer.json\`). Re-run the installer to change an answer. Read this after \`USER.md\`, before your first substantive reply._

### Who you are working with

- **Call them:** ${esc(a.name)}${a.pronouns ? ` (${esc(a.pronouns)})` : ''}
${a.work ? `- **What they do:** ${esc(a.work)}\n` : ''}${a.focus ? `- **Current focus:** ${esc(a.focus)}\n` : ''}- **How firmly they mean things:** read the rungs in \`USER.md\` § How firmly I mean things, and default one rung looser when unsure.
...
- **When you are wrong:** ${label(Q.mistakes, a.mistakes)}.
- **When you are unsure:** ${label(Q.unsure, a.unsure)}.
- **Never:** ${a.never.length ? a.never.map(esc).join('; ') : 'nothing declared'}.
...
${bullets(a.read_first.map(esc), 'USER.md')}
${a.tracker !== 'none' ? `- The task tracker (${esc(a.tracker)}): what is open and what is already decided, before proposing work.\n` : ''}
### Where you may write
...
${bullets(a.off_limits.map(x => `${esc(x)}: never surfaced, quoted or summarized in any reply or deliverable, even if a search returns it`), 'nothing declared yet')}
...
function contractBlock(a) {
  return [
    ...
    a.never.length ? `Never: ${a.never.map(esc).join('; ')}.` : null,
    `Always ask before: ${a.always_ask.join(', ') || 'nothing declared'}.`,
    a.off_limits.length ? `Off limits in any output: ${a.off_limits.map(esc).join(', ')}.` : null,
    `Read first: ${a.read_first.map(esc).join(' → ')}.`,
  ].filter(Boolean).join('\n');
}
