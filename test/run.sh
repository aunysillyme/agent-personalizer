...
mk() { d="$(mktemp -d)" || fail "mktemp -d failed"; [ -n "$d" ] && [ -d "$d" ] || fail "mktemp gave no directory"; TMPS="$TMPS $d"; echo "$d"; }
...
T="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"
...
T="$(mk)"; O="$(mk)"; echo "seeded secret name" > "$O/outside.md"; ln -s "$O" "$T/link"; echo "seeded secret name" > "$T/list.txt"
...
T="$(mk)"; expect 2 "installer junk level" node bin/agent-personalizer.js --dir "$T" --ai claude --level 3garbage --yes
...
T="$(mk)"; O="$(mk)"; ln -s "$O/created-by-installer.md" "$T/USER.md"
...
T="$(mk)"; O="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; printf 'SENTINEL\n' > "$O/victim.md"; cp "$O/victim.md" "$O/victim.expected"
...
T="$(mk)"; O="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; mkdir -p "$T/render"; cp render/render.js "$T/render/"
printf 'SENTINEL\n' > "$O/victim.md"; cp "$O/victim.md" "$O/victim.expected"
...
T="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; printf '\n<!-- agent-personalizer:end -->\n' >> "$T/USER.md"
...
T="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; sed -i.bak 's/^surfaces: \[claude, agents, gemini\]$/surfaces: [claude, agents/' "$T/rules/20-one-owner-per-rule.md"; rm -f "$T/rules/"*.bak
expect 2 "unclosed surfaces" node render/render.js --dir "$T" --check
T="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; sed -i.bak 's/^surfaces: \[claude, agents, gemini\]$/surfaces: [claude, nowhere]/' "$T/rules/20-one-owner-per-rule.md"; rm -f "$T/rules/"*.bak
...
node render/render.js --dir "$T" --contract --contract-target claude | grep -q PROMPTONLYRULE && fail "claude contract included a prompt-only rule"
node render/render.js --dir "$T" --contract --contract-target prompt | grep -q PROMPTONLYRULE || fail "prompt contract missed its own rule"
node render/render.js --dir "$T" --contract --contract-target prompt --no-personal | grep -q PERSONALBLOCK && fail "--no-personal leaked a personal block"
pass "contract respects surfaces and --no-personal"

# 18. hook exits non-zero when node is absent, zero when present
T="$(mk)"; cp -R examples/freelance-illustrator/. "$T/"; mkdir -p "$T/render" "$T/hooks/claude-code"; cp render/render.js render/targets.json "$T/render/"; cp hooks/claude-code/session-start.sh "$T/hooks/claude-code/"
sh "$T/hooks/claude-code/session-start.sh" >/dev/null 2>&1 || fail "hook failed with node present"
if PATH=/nonexistent sh "$T/hooks/claude-code/session-start.sh" >/dev/null 2>&1; then fail "hook exited 0 with node absent"; fi
pass "hook: injects with node, fails loud without it"
