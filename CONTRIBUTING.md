...
```bash
npm test            # sh test/run.sh: 60 checks, exact exit codes, Linux and macOS
node test/rollback.test.js "$(mktemp -d)"   # after copying examples/freelance-illustrator into it
node check/gate.js --self-test
```

The privacy gate (`check/gate.js` against a real list) needs a local `check/forbidden.local.txt`; copy the example and fill it with *your* identifiers. It never ships and the harness prints that check as a skip when the list is absent. A skip is not a pass; say so in your PR if you could not run it.
...
- A notes tool is an entry in the `TOOL` map in `render/onboarding.js` with a `kind` (`disk`, `cloud`, `readonly`, `other`) and a row in `docs/companions.md`. Check 60 in the harness iterates every option; add yours to the right loop.
...
- If you touched `render/render.js`, `bin/agent-personalizer.js` or `check/gate.js`, say what you attacked: a symlink, a traversal, a malformed marker, a bad answer. The PR template asks.
