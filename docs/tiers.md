...
**Prevents.** Decay. A style stated once in a message fades after twenty turns. A style in the system prompt is re-sent on turn two hundred exactly as on turn one, so it cannot fade for lack of presence (it can still be ignored).
...
**Rule inside the tier.** The home file carries pointers, or a generated block, never a hand-maintained copy. A `[hook: name]` or `[owner: path]` tag on a one-line rule tells the AI where the full text lives. The block between the render markers is the one exception: it is a generated snapshot of `USER.md` and `rules/`, regenerated by `render/render.cjs` and compared by `--check`, so it has one owner and a drift detector. What this tier forbids is a second copy you maintain by hand.
...
**Prevents.** A tool's own instructions beating yours by default. Installed skills, plugins and tool descriptions each say "use me." A rule that says "use the other one first" loses that argument if it is only stated in a document the AI may or may not have opened. Injection makes sure your rule is at least in the room; it does not settle the argument on every host.
...
**Rule inside the tier.** If you find yourself pasting a rule's text into a second file by hand, stop and write a pointer, or let the renderer generate the copy and check it. Pasted copies you cannot check (a chat app's instruction box) are re-pasted after every change; nothing can detect a stale paste for you.
